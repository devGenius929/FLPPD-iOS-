...
private let reuseIdentifier = "connectionCell"

class ConnectionsViewController: UICollectionViewController {

    var connections = [UserAndMutuals]()
    var user:User? {
        didSet {
            if user != nil {
                loadConnections()
    ...
    internal func loadConnections() {
        guard  let user = self.user else {
            return
        }
        ClientAPI.default.getFriensForUser(withId: user.user_id) {[weak self] (users, error) in
            if let users = users {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.configureCell(user: connection)
        return cell
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let vc = UIStoryboard(name: "Profile", bundle: nil).instantiateViewController(withIdentifier: "UserProfileViewController") as! UserProfileViewController
         let connection = connections[indexPath.row]
        vc.loadUserWithId(connection.user.user_id)
