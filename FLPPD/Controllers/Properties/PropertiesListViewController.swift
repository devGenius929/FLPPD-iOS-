...
class PropertiesListViewController: UIViewController {
  ...
  private let disposeBag = DisposeBag()
    @IBOutlet weak var proButton: CurvledButton!
    @IBOutlet weak var buyButton: CurvledButton!
    
    @IBAction func proAction(_ sender: Any) {
    }
    @IBAction func BuyListing(_ sender: Any) {
    }
  ...
  func getProperties(){
    ...
    guard let observable = getPropertiesObservable(user_id) else{
      return
    }
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let action = UITableViewRowAction(style: .destructive, title: "Delete") { [weak self] (action, indexPath) in
            if let property = self?.properties.value[indexPath.row]{
                ClientAPI.default.deleteProperty(property.flppdProperty.id, completion: { (success) in
                    if success {
                        self?.getProperties()
                    }
                })
